...
export function ServicesPricing() {
  ...
  const ladiesServices = [
    { name: "Shampoo + Haircut + Blowdry", price: "₹200" },
    ...
    { name: "Straightening / Smoothening", price: "₹3000 - ₹3500" },
        ...
        <div className="flex flex-col bg-surface/30 p-8 md:p-12 rounded-[2rem] border border-white/5 shadow-2xl">
          ...
          <div className="flex flex-col gap-6">
            {gentsServices.map((item, idx) => (
              <div key={idx} className="group flex justify-between items-end border-b border-white/5 pb-4 hover:border-accent/40 transition-colors duration-300">
                <h4 className="font-serif text-xl md:text-2xl text-foreground/90 group-hover:text-white transition-colors duration-300 pr-4">
                  {item.name}
                </h4>
                ...
                <p className="font-medium text-accent text-lg md:text-xl whitespace-nowrap">
                  {item.price}
                </p>
              </div>
            ))}
